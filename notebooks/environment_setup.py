    ...
    def _initialize_patterns(self):
        ...
        if "hour" in self.data.columns and "ACTIVITY_NAME_ENC" in self.data.columns:
            ...
            for activity in self.data["ACTIVITY_NAME_ENC"].unique():
                ...
                hour_dist = (
                    activity_data["hour"].value_counts(normalize=True).sort_index()
                )
...
class ScheduleEnv(gym.Env):
    ...
    def __init__(self, data, num_slots=24):
        ...
        if "ACTIVITY_NAME" in data.columns:
            ...
            for act_code in self.unique_activities:
                act_name = data[data["ACTIVITY_NAME_ENC"] == act_code][
                    "ACTIVITY_NAME"
                ].iloc[0]
            ...
            self.activity_names = {
                act: f"Activity {act}" for act in self.unique_activities
            }
        ...
        obs_dim = (
            3 + self.num_slots
        )  # slot actuel + jour + dernière action + activités planifiées
    ...
    def _get_observation(self):
        ...
        last_act_normalized = (
            self.last_activity / (self.num_activities - 1)
        ...
        observation = np.concatenate(
            [[slot_normalized, day_normalized, last_act_normalized], planned_activities]
        )
    ...
    def _calculate_reward(self, action, time_slot):
        ...
        time_pref_score = self.activity_patterns.get_time_preference_score(
            action, time_slot
        )
        ...
        if time_slot > 0:
            ...
            sequence_score = self.activity_patterns.get_activity_sequence_score(
    ...
    def render(self, mode="human"):
        ...
        if mode == "human":
            ...
            print(
                f"📅 Jour: {['Dim', 'Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam'][self.day_of_week]}"
            )
